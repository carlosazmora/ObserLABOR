...
def ejecutar_con_progreso(funcion_proceso, mensaje_inicial: str, container):
    """
    Ejecuta una función mostrando progreso basado en logs tipo [x/total].

    - container: columna o contenedor donde se renderiza el progreso
    """
    ...
    try:
        ...
        with container.expander("📋 Ver log completo"):
            st.text("\n".join(log_lines))
...
def mostrar_habilidades() -> None:
    ...
    with st.expander("🔄 Gestión de datos O*NET", expanded=not bd_tiene_datos()):
        ...
        if bd_tiene_datos():
            ...
            if actualizar:
                def proceso(log_fn):
                    pipeline_datos(log_fn=log_fn)
                    st.cache_data.clear()

                ejecutar_con_progreso(
                    proceso,
                    "Procesando datos O*NET...",
                    col_boton  # 👈 CLAVE: aquí defines dónde se renderiza
                )
        else:
            with col_estado:
        ...
        if actualizar:
            def proceso(log_fn):
                pipeline_datos(log_fn=log_fn)
                st.cache_data.clear()
            ejecutar_con_progreso(proceso, "Procesando datos O*NET...")
